...
namespace ModInventario.src.Filtro.FiltroRep.ModoSoloReporte
{
    ...
    public class ImpModo: baseFiltro, IModo
    {
        ...
        public override bool CargarData()
        {
            if (base.CargarData())
            {
                try
                {
                    ...
                    var r05 = Sistema.MyData.Empresa_Sucursal_TipoPrecioManejar(Sistema.Negocio.CodigoEmpresa);
                    var _idPrecioManejar = r05.Entidad;

                    var lPrecio = new List<ficha>();
                    var r04 = Sistema.MyData.Sistema_TipoPreciosDefinidos_Lista();
                    foreach (var rg in r04.Lista.Where(w=>w.id==_idPrecioManejar).OrderBy(o => o.descripcion).ToList())
                    {
