...
{
    public class ImpAgregar: ModoSucursal.BaseAgregarEditarModoSucursal, IAgregar
    {
        public override string GetTitulo { get { return "Agregar/Crear Ficha Adm"; } }
        ...
        private void GuardarFicha()
        {
            ...
            try
            {
                ...
                var _lstHndEmpVenta = new List<OOB.LibInventario.Producto.Agregar.HndEmpVenta>();
                var emp1 = new OOB.LibInventario.Producto.Agregar.HndEmpVenta()
                {
                    autoEmp = _data.GetEmpVentaTipo1.id,
                    contEmp = _data.GetContEmpVentaTipo1,
                    tipoEmp = "1",
                };
                var emp2 = new OOB.LibInventario.Producto.Agregar.HndEmpVenta()
                {
                    autoEmp = _data.GetEmpVentaTipo2.id,
                    contEmp = _data.GetContEmpVentaTipo2,
                    tipoEmp = "2",
                };
                var emp3 = new OOB.LibInventario.Producto.Agregar.HndEmpVenta()
                {
                ...
                ficha.hndEmpVenta = new OOB.LibInventario.Producto.Agregar.FichaHndEmpVenta()
                {
                    ListaEmpVenta = _lstHndEmpVenta,
                };

                var _fechaNula = new DateTime(2000, 1, 1);
                var r02 = Sistema.MyData.Sistema_TipoPreciosDefinidos_Lista();
                ficha.hndPrecioVenta = null;
                var _lstHndPrecioVenta = new List<OOB.LibInventario.Producto.Agregar.HndPrecioVenta>();
                foreach(var rg in r02.Lista)
                {
                    for (var y = 0; y < 3; y++)
                    {
                        var _precioVenta = new OOB.LibInventario.Producto.Agregar.HndPrecioVenta()
                        {
                            fullDivisa = 0m,
                            idHndTipoPrecio = rg.idValue,
                            netoMonedaLocal = 0m,
                            ofertaDesde = _fechaNula,
                            ofertaEstatus = "0",
                            ofertaHasta = _fechaNula,
                            ofertaPorc = 0m,
                            tipoEmp = (y + 1).ToString().Trim(),
                            utilidadPorc = 0m,
                        };
                        _lstHndPrecioVenta.Add(_precioVenta);
                    }
                }
                ficha.hndPrecioVenta = new OOB.LibInventario.Producto.Agregar.FichaHndPrecioVenta()
                {
                    ListaPrecioVenta = _lstHndPrecioVenta,
                };

                var r03 = Sistema.MyData.Producto_Nuevo_Agregar(ficha);
